	...
	public function update(Level $level){
		...
		for($x = $this->minX; $x <= $this->maxX; ++$x){
			...
			for($z = $this->minZ; $z <= $this->maxZ; ++$z){
				...
				if(!isset($lightsa[PMFLevel::getIndex(($x-1) >> 4, ($z) >> 4)])){
					console("skipping");
					continue;
				}
				if(!isset($lightsa[PMFLevel::getIndex(($x+1) >> 4, ($z) >> 4)])){
				...
				if(!isset($lightsa[PMFLevel::getIndex(($x) >> 4, ($z-1) >> 4)])){
					console("skipping");
					continue;
				}
				if(!isset($lightsa[PMFLevel::getIndex(($x) >> 4, ($z+1) >> 4)])){
				...
				for($y = $this->minY; $y <= $this->maxY; ++$y){
					...
					if($isBlockLight){
						...
						if($level->isSkyLit($x, $y, $z)) $lightEmission = 15;
					}
					...
					if($lightBlock <= 14 || $lightEmission != 0){
						...
						if($upper){
							$yNeg = $brightness_low;
							$yPos = ord($lights[($bindex+1) >> 1]) & 0xf;
						}else{
						...
						console("$isBlockLight $id $lightEmission $newBrightness");
					}else{
						$newBrightness = 0;
